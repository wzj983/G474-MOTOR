...
/**
  * @brief  Switch Primary - Auxiliary sensor.
  *
  * @param  pHandle Pointer on the component instance to work on.
  * @param  deltaAngleMargin is the maximum error betwen Primary and Auxiliary sensor to allow the switch
  *
  * @retval returns true if the command is successfully executed, false otherwise.
  */
__weak bool MCI_SensorSwitch(MCI_Handle_t *pHandle, uint16_t deltaAngleMargin)
{
  bool status = false;
#ifdef NULL_PTR_CHECK_MC_INT
  if (MC_NULL == pHandle)
  ...
  {
#endif
	SpeednPosFdbk_Handle_t *newSensor;
    newSensor = pHandle->pSTC->AUXSPD;
    status = STC_SensorSet(pHandle->pSTC, pHandle->pFOCVars->AuxDeltaAngle,deltaAngleMargin,newSensor );
#ifdef NULL_PTR_CHECK_MC_INT
  }
#endif
 return status;
...
/**
  * @brief  Set Primary sensor used by FOC algo
  *
  * @param  pHandle Pointer on the component instance to work on.
  * @param  deltaAngleMargin is the maximum error betwen Primary and Auxiliary sensor to allow the switch
  * @param  sensor becomes the primary sensor used by FOC algo if the command is successful.
  *
  * @retval returns true if the command is successfully executed, false otherwise.
  */
__weak bool MCI_SensorSet(MCI_Handle_t *pHandle, uint16_t deltaAngleMargin, SpeednPosFdbk_Handle_t *sensor)
{
  bool status = false;
#ifdef NULL_PTR_CHECK_MC_INT
  if (MC_NULL == pHandle)
  ...
  {
#endif
   if (RUN == pHandle->State ){
     status = STC_SensorSet(pHandle->pSTC, pHandle->pFOCVars->AuxDeltaAngle,deltaAngleMargin,sensor );
   }
   else
   {
     /* Nothing to do */
