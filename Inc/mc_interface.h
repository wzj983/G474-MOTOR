...
typedef struct
{
 ...
 uint16_t hDurationms;                  /*!< Duration in ms of last ExecSpeedRamp or ExecTorqueRamp command.*/
  ...
  qd_t Iqdref;                           /*!< Current component of last SetCurrentReferences command.*/
   uint16_t deltaAngleMargin;            /*!< Max Delta angle authorized to allow sensor switch*/
   SpeednPosFdbk_Handle_t *newSensor;    /*!< new sensor to set as primary sensor for FOC algo*/
...
bool MCI_SensorSwitch(MCI_Handle_t *pHandle, uint16_t deltaAngleMargin);
bool MCI_SensorSet(MCI_Handle_t *pHandle, uint16_t deltaAngleMargin, SpeednPosFdbk_Handle_t *sensor);
/**
  * @}
  */
